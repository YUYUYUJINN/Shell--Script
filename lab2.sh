...
temp=$(mktemp -t test.XXX) # 함수내에서 결과를 파일로 저장하기 위해
...
vmcreation(){
	...
	*)
	...
	then
		...
		# 종료 코드가 0(OK) 인 경우 flavor 선택으로 이동하기
		if [ $? -eq 0 ]
		then
			dialog --title "스펙 선택" --radiolist "필요한 자원을 선택하세요" 15 50 5 "mi.small" "가상 CPU 1개, 메모리 1GB" ON "m1.medium" "가상 CPU 2개, 메모리 2GB" OFF "m1.large" "가상 CPU 4개, 메모리 8GB" OFF 2> $flavor
...
# 가상머신 삭제 함수
vmdelete(){
	temp2=$(for var in $(virsh list --all | grep -v Id | gawk '{print $2}' | sed '/^$/d'); do echo -n "$var ...  ON "; done)
	dialog --title "삭제할 가상머신 선택하기" --radiolist "삭제할 가상머신을 아래에서 선택하세요" 15 50 5  $temp2 2> $vmname
	...
	if [ $? -eq 0 ]
	then
		dialog --title "삭제확인" --yesno "삭제하시겠습니까?" 10 20

		
		if [ $? -eq 0 ]
		then
			virsh destroy ${delname}
			virsh undefine ${delname} --remove-all-storage
			dialog --msgbox "삭제가 완료되었습니다" 10 50
